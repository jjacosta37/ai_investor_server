...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            if not securities:
                self.stdout.write(
                    self.style.WARNING('No securities found to process')
                )
                return
            ...
            for i, security in enumerate(securities, 1):
                try:
                    ...
                    if options['dry_run']:
                        self.stdout.write(
                            self.style.WARNING(f'DRY RUN: Would process {security.symbol}')
                        )
                        processed_count += 1
                        continue
                    ...
                    if not analysis:
                        self.stdout.write(
                            self.style.ERROR(f'  ✗ Failed to get analysis for {security.symbol}')
                        )
                        error_count += 1
                        continue
                    ...
                    if not analysis_service.validate_analysis(analysis):
                        self.stdout.write(
                            self.style.ERROR(f'  ✗ Invalid analysis data for {security.symbol}')
                        )
                        error_count += 1
                        continue
