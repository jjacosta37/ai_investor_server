...
MAX_UPCOMING_EVENTS_PER_SECURITY = getattr(settings, 'MAX_UPCOMING_EVENTS_PER_SECURITY', 20)
...
class NewsDataTransformer:
    ...
    def save_analysis_to_db(
        ...
        try:
            ...
            sentiment, sentiment_created = OverallSentiment.objects.get_or_create(
                ...
                defaults={
                    'confidence_level': getattr(analysis.overall_sentiment, 'confidence_level', None)
                }
            )
            ...
            for order, highlight in enumerate(analysis.key_highlights):
                KeyHighlight.objects.create(
                    security_news_summary=news_summary,
                    highlight=highlight,
                    order=order
                )
            ...
            for news_data in analysis.recent_news:
                ...
                if not NewsItem.objects.filter(url=news_data.url).exists():
                    ...
                    if not parsed_date:
                        ...
                        logger.warning(f"Using today's date for news item: {news_data.headline}")
            ...
            if not skip_cleanup:
                effective_max_news = max_news_items if max_news_items is not None else MAX_NEWS_ITEMS_PER_SECURITY
                NewsDataTransformer.cleanup_excess_news_items(security, effective_max_news)
            ...
            if not skip_cleanup:
                effective_max_events = max_events if max_events is not None else MAX_UPCOMING_EVENTS_PER_SECURITY
                NewsDataTransformer.cleanup_excess_upcoming_events(security, effective_max_events)
    ...
    def get_watchlisted_securities() -> List[Security]:
        ...
        try:
            ...
            security_ids = WatchlistItem.objects.values_list('security_id', flat=True).distinct()
    ...
    def cleanup_excess_news_items(security: Security, max_items: int):
        ...
        try:
            ...
            if total_count > max_items:
                ...
                oldest_items = NewsItem.objects.filter(security=security)\
                    .order_by('created_at')\
                    .values_list('id', flat=True)[:excess_count]
                
                # Delete the oldest items
                deleted_count = NewsItem.objects.filter(id__in=list(oldest_items)).delete()[0]
    ...
    def cleanup_excess_upcoming_events(security: Security, max_items: int):
        ...
        try:
            ...
            if total_count > max_items:
                ...
                # Get IDs of oldest items to delete
                oldest_items = UpcomingEvent.objects.filter(security=security)\
                    .order_by('created_at')\
                    .values_list('id', flat=True)[:excess_count]
                
                # Delete the oldest items
                deleted_count = UpcomingEvent.objects.filter(id__in=list(oldest_items)).delete()[0]
            ...
            logger.error(f"Error cleaning up upcoming events for {security.symbol}: {e}")